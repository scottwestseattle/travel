...
<div class="container">
	...
	<form method="POST" action="/{{$prefix}}/filter">
		...
		<h3>Trades ({{count($records)}}), Total: ${{round($totals['total'], 2)}} {{ isset($totals['reconciled']) ? ', Reconciled: ' . round($totals['reconciled'], 2) . '' : '' }}</h3>
			...
			@if (isset($records))
				...
				@foreach($records as $record)
					...
					@if (App\Transaction::isBuyStatic($record))
						<td class="glyphCol"><a href='/{{$prefix}}/add-trade/{{$record->lot_id}}'>Sell</a></td>
					@else
						<td class="glyphCol"></td>
					@endif
